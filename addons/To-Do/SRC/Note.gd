...
const TASK_BG_ALPHA:float = 0.04
...
func _ready():
	new_stylebox_normal = task_chooser.get_stylebox("normal").duplicate()
	new_stylebox_disabled = task_chooser.get_stylebox("disabled").duplicate()
...
func set_colour_tag(index:int):
	...
	task_chooser.add_stylebox_override("normal", new_stylebox_normal)
...
func set_completed_color_tag(color_tag):
	...
	task_chooser.add_stylebox_override("disabled", new_stylebox_disabled)
