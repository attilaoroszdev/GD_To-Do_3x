...
func _on_AddButton_pressed():
	if task_edit.text != "" :
		...
		ins.connect("state_changed", self, "task_state_changed")
		ins.connect("content_changed", self, "save_changes")
		
		# A ton of new signals for interaction with different parts
		ins.connect("task_removed", self, "remove_task")
		ins.connect("move_up", self, "move_task_up")
		ins.connect("move_down", self, "move_task_down")
...
func move_task_up(ins):
	var pos = ins.get_position_in_parent()
	# Probably unnecessay, button should be disabled anyway
	if pos > 0:
